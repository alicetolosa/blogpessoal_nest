...
import request from 'supertest'; //incluido
...
describe('Testes dos Módulos Usuário e Auth (e2e)', () => {  // descrição do teste e2e
  ...
  it("01 - Deve criar um novo usuario", async () =>{ //testa se o usuario pode ser criado
    ...
    .post('/usuarios/cadastrar').send({
      nome: 'alice',
      usuario: 'aolive@gmail.com',
      senha: '12345678',
      foto: '-'
    }).expect(201)// resposta com o retorno 201
  ...
  it("05 - Deve Atualizar o Usuario", async () => {
      ...
      .send({
      id: usuarioid, 
      nome: 'alice Atualizado',
      usuario: 'alive@gmail.com',
      senha: '12345678',
      foto: '-',
      })
      .expect(200)
      .then((resposta) => {
        expect('alice Atualizado').toEqual(resposta.body.nome);
